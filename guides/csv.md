...
hand-written tokenizer and table builders `parsers.ts` composes, the flat cell
coercers, and the rendering projections callers reach for directly (AGENTS §5).
...
The parse entry point, from [`parsers.ts`](../src/core/parsers.ts) — the one
function that composes the `helpers.ts` tokenizer, the `helpers.ts` table
builders, and the `inferers.ts` column inference into a `CSVParseResult`.
...
- [`../../tests/src/core/helpers.test.ts`](../tests/src/core/helpers.test.ts) —
  ...
  narrowing, and `renderCSV`/`renderTSV`.
