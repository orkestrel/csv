...
// Guards here are total - never throw, return `false` for any off-shape
// input. `isCSVTable` validates arbitrary `unknown` input (a deserialized
// table, a value crossing a process boundary) against the CSVTable structure
// (types.ts), delegating its row check to `@orkestrel/contract`'s `isRecord`
 ...
 *
 * @param value - The value to test
 * @returns True if `value` is a {@link ColumnType} literal; false otherwise
 *
 * @example
 * ```ts
